...
def train_parallel(args):
    ...
    print(f"Training on {device} | Mode: {mode_str} | Reward: {reward_str} | Opponents: {opp_str}" + (f" (={len(opponent_models) + num_builtin} total)" if getattr(args, 'slot', None) else ""))
    ...
    project = getattr(args, 'wandb_project', None) or 'hockey-sac-parallel'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--slot', type=str, default=None, help='Slot name, e.g. sac-default (required for round-based)')
