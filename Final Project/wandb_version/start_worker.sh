#!/usr/bin/env bash
# Start a worker: ensure conda env and deps, then run worker with training.yaml.
# Usage: ./wandb_version/start_worker.sh [optional args, e.g. --poll_interval 10]
set -e
...
cd "$PROJECT_ROOT"
echo "Running worker (config: $CONFIG)"
exec conda run -n "$CONDA_ENV" python "$SCRIPT_DIR/worker.py" --config "$CONFIG" "$@"
